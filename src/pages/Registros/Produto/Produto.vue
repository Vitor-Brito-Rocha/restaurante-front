<template>
  <div class="w-100 h-100">
    <div class="w-100 d-flex gap-4 align-top justify-end">
      <v-btn v-tooltip="'Alternar visão'" size="small" icon="mdi-swap-vertical" @click="viewMenu = !viewMenu" />
    ...
    <div class="h-75 mt-2 mb-2 w-100">
      <CommomTableList v-if="!viewMenu" :data="items" :headers="headers" :permissoes="permissoes" :perPage="offset" :total-items="totalItems" :page="page" :loading="loadingTable" @verify="verifyGetFunction($event)" @deleteModal="deletarProduto($event)" @updateStatus="updateStatus($event)" @editModal="editViewModal" />
